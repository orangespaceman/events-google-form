(function(config) {
  ...
  function init() {
    ...
    inputEls = formEl.querySelectorAll('.Form-input, .Form-textarea, .Form-select');
    selectEl = formEl.querySelector('.Form-select');
    ...
    selectEl.addEventListener('change', checkInputValidity);
  ...
  function prepareFormData(latLon) {
    ...
    inputEls.forEach(function(inputEl) {
      if (inputEl.classList.contains("Form-select")) {
        var value = inputEl.options[inputEl.selectedIndex].value;
        data.push(config.form[inputEl.id] + '=' + encodeURIComponent(value));
      } else {
    ...
    data.push(config.form.visible + '=false');
